...
public:
	...
	bool moveSubtree(NodeDynamic<double>* parentNode, NodeDynamic<double>* newChildNode) {
		if (parentNode == NULL || newChildNode == NULL) {
			std::cerr << "Null pointers\n";
			return false;
		}
	...
	int sumTree() {
		return recursiveSumTree(root);
	}

private:
	NodeDynamic<double>* root;
	int recursiveSumTree(NodeDynamic<double>* node) {
		double sum = node->value;

		for (int i = 0; i < node->getChildrenNumber(); i++) {
			sum += recursiveSumTree(node->getChild(i));
		}
